$(function(){
	...
	$('.firstName,.lastName,.emailAddress,.comfirmEmail,.date,.currentPwd,.pwd,.confirmPwd').blur(function(){
		...
		if ($(this).hasClass('firstName')) {
			if (regular) {
				input(this);
				$(this).siblings('p').text('No Spaces, 3-16 characters long, must contain at least two varters, Numbers, and symbols.').fadeIn('slow');
			}else {
				input1(this);
			}
		}
    ...
    // 以下是点击.centerLeft下li后右边显示相对应的内容
	    ...
	    $('.popupClose').hover(function() {
	    	$(this).children('img').attr('src','images/member/delete_normal1.png');
	    }, function() {
	    	$(this).children('img').attr('src','images/member/delete_normal.png');
	    });
